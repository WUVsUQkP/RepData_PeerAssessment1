...
Firstly, sum steps by date and store value in a new object called "stepsDate":

```{r stepsbyday}
stepsDate<-tapply(mydata$steps,mydata$date,sum)
```

Secondly, remove NA values and store remaining values in an object called "dailysteps":

```{r removeNA}
dailysteps<-stepsDate[!is.na(stepsDate)]
```

Thridly, plot histogram of total steps per day:

```{r histSteps}
hist(dailysteps)
```

### 2. Calculate and report the **mean** and **median** total number of steps taken every day.
...
Firstly, remove all NA values from Steps column:
```{r removeNA2}
badrows<-is.na(data$steps)
good.data<-data[!badrows,]
```

Secondly, average steps by interval (across all days) and store in an object called stepsInterval:
```{r stepsInterval}
stepsInterval<-tapply(good.data$steps,good.data$interval,mean)
```

Thirdly, create an object that contains all unique Interval values:

````{r Interval}
Interval<-unique(data$interval)
```

Fourthly, plot Interval against stepsInterval:
...
Firstly, combine the stepsInterval and Interval objects into a single dataframe:

```{r df}
df<-data.frame(stepsInterval,Interval)
```

Secondly, find the Interval value that corresponds to the max stepsInterval value:
...
Firstly, get an understanding of where the NA values are:
...
I am going to use the median value for the corresponding interval, across all days. 

Thus firstly, create an array that contains all the median step values across all days for each interval. The median values for each interval need to be repeated for each date so that the length of the new data frame is the same length as the original data set:
...
Secondly, create an integer vector that contains a list of all the rows in data$steps that need replacement values:
```{r replacementRows}
...
Firstly, sum steps by date and store value in a new object called "stepsDate2":
